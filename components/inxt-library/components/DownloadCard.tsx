...
import { Confetti, FileArrowDown } from '@phosphor-icons/react';
...
const SuccessScreen = ({ textContent }) => {
  return (
    <>
      <Confetti size={96} className="text-primary" />
      ...
      <p className="text-center text-xl text-gray-80 lg:text-left">
        {textContent.subtitle1} <span className="font-semibold">{textContent.subtitle2}</span>
      </p>
    </>
  );
};
...
const DownloadCard = ({ textContent, imageUrl, imageAlt, bookUrl }) => {
  ...
  const [bookDownloaded, setBookDownloaded] = useState(false);

  const screen = bookDownloaded ? (
    <SuccessScreen textContent={textContent.successMessage} />
  ) : (
    <>
