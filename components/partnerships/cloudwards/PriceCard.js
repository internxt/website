    ...
    <div className={`priceCard card ${popular ? 'bg-blue-60 ring-2 ring-blue-60 shadow-lg' : ''} flex flex-col flex-shrink-0 flex-grow-0 max-w-xs rounded-2xl overflow-hidden m-2 sm:m-4`}>
      ...
      <div className={`info flex flex-col p-6 items-center justify-center bg-white ${popular ? 'rounded-t-2xl' : ''}`}>
        ...
        <div className={`storage flex flex-row whitespace-nowrap py-1 pb-0.5 px-4 max-w-min ${popular ? 'bg-blue-10 text-blue-60' : 'bg-neutral-20 text-neutral-80'} font-semibold rounded-full`}>
          <p>
            {storage}
            <span className={`${planType.toLowerCase() === 'individual' ? 'hidden' : ''} text-sm`}>{contentText.perUserSlash}</span>
          </p>

        </div>

        <div className={`planPrice flex flex-col py-8 justify-center items-center ${priceBefore ? 'space-y-1' : 'space-y-2'}`}>
          ...
          <span className={`priceBefore ${priceBefore ? 'flex' : 'hidden'} text-base text-neutral-80 font-medium line-through`}>
            €
            {priceBefore}
          </span>
