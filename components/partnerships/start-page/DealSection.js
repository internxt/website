...
const DealSection = ({ textContent }) => {
  return (
    <section className="overflow-hidden bg-gray-1 px-8 py-20 md:px-32 xl:px-56">
      <div className="flex flex-col justify-center space-y-5 lg:flex-row lg:space-x-40">
        {/* Text */}
        <div className="flex flex-col items-center justify-center space-y-8 lg:items-start lg:whitespace-nowrap">
          <p className="text-center text-5xl font-semibold lg:text-start">{textContent.title}</p>
          <div className="flex flex-col space-y-4">
            {textContent.adv.map((adv) => (
              <div className="flex flex-row items-start space-x-4 whitespace-pre-wrap lg:items-center">
                <Check className="text-primary" size={25} weight="bold" />
                <p className="text-xl font-normal">{adv.title}</p>
              </div>
            ))}
          </div>
          <p className="text-lg font-normal text-gray-60">{textContent.footer}</p>
        </div>
        {/* Card */}
