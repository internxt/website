  ...
  return (
    <div className={`priceCard ${popular ? 'bg-blue-60 ring-4 ring-blue-60' : ''} flex flex-col flex-shrink-0 flex-grow-0 rounded-2xl shadow-xl overflow-hidden m-2`}>
      ...
      <div className={`info flex flex-col p-6 items-center justify-center bg-white ${popular ? 'rounded-t-2xl' : ''}`}>
        ...
        <div className={`businessUserCount ${planType.toLowerCase() === 'individual' ? 'hidden' : 'flex'} flex-col w-full bg-neutral-10 ring-1 ring-neutral-20 rounded-lg p-4 mb-2`}>
          
          <div className="input relative flex flex-row justify-between bg-white rounded-lg ring-1 ring-neutral-30 overflow-hidden">
            <button onClick={() => { if (getUsers >= 3) { setUsers(parseInt(getUsers) - 1) } else ( setUsers(2) ) }} className={`flex flex-row items-center justify-center h-12 w-12 sm:h-8 sm:w-8 bg-blue-60 text-white text-2xl z-10`}>-</button>
            <button onClick={() => { if (getUsers <= MAX_USERS - 1) { setUsers(parseInt(getUsers) + 1) } else ( setUsers(MAX_USERS) ) }} className={`flex flex-row items-center justify-center h-12 w-12 sm:h-8 sm:w-8 bg-blue-60 text-white text-2xl z-10`}>+</button>
            <label for={`users_${storage}`} className={`absolute top-0 left-0 h-full w-full flex flex-row items-center justify-center text-xl sm:text-base font-medium cursor-text`}>
              <input id={`users_${storage}`} type="number" min="2" max={MAX_USERS} step="1" value={getUsers} onChange={(e) => { setUsers( (e.target.value > MAX_USERS) ? MAX_USERS : e.target.value ) }} onBlur={(e) => { setUsers( (e.target.value > MAX_USERS) ? MAX_USERS : (e.target.value < 2 ? 2 : e.target.value) ) }} className={`text-right w-min appearance-none outline-none`} />
      ...
      <div className="featureList flex flex-col p-6 text-neutral-500 bg-neutral-10 border-t border-neutral-20">
        <div className="flex flex-col space-y-2">
          <div className={`flex flex-row space-x-2 font-semibold`}><img loading="lazy" src="/icons/checkNeutral500.svg" draggable="false"/><span className={`${price <= 0 ? 'hidden' : 'flex'}`}>{billingFrequency < 0 ? `Enjoy ${storage} forever!` : '30 days money-back guarantee'}</span><span  className={`${price <= 0 ? 'flex' : 'hidden'}`}>Enjoy {storage} for free, forever!</span></div>
          <div className="flex flex-row space-x-2"><img loading="lazy" src="/icons/checkNeutral500.svg" draggable="false"/><span>Encrypted file storage and sharing</span></div>
          ...
          <div className="flex flex-row space-x-2"><img loading="lazy" src="/icons/checkNeutral500.svg" draggable="false"/><span>Get access to all our services</span></div>
        </div>
      </div>
