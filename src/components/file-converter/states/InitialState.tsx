...
const InitialState = ({ handleFileDrop, isDragging, setIsDragging, handleOpenFileExplorer }: InitialStateProps) => {
  return (
      ...
      <div className="flex flex-col items-center space-y-8">
        {isDragging ? (
          <div className="flex flex-col items-center space-y-4">
            <EmptyFile />
            <p className="text-2xl font-semibold">Drop file here</p>
          </div>
        ) : (
          <>
            ...
            <button className="flex rounded-lg bg-primary px-5 py-2.5 text-white" onClick={handleOpenFileExplorer}>
              <p className="font-medium">Choose X file</p>
            </button>
          </>
        )}
      </div>
