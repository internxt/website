...
export interface NASPageProps {
  ...
  metaTagId: string;
  /** Breadcrumb trail; the landing stops at /nas, the vendor pages add their own leaf. */
  breadcrumbItems: { name: string; url: string }[];
  /** Only the /nas landing emits the FAQ schema markup. */
  includeFaqSchema?: boolean;
  /** Synology drops the desktop reset and the fixed width on the first CTA. */
  firstCtaTextPadding?: string;
  firstCtaDescriptionClass?: string;
}
...
export const NASPage = ({
  ...
  firstCtaTextPadding = 'px-4 lg:px-0',
  firstCtaDescriptionClass = 'text-base font-normal leading-tight text-gray-55 lg:w-[633px] lg:text-center lg:text-xl',
  ...
  return (
    <>
      <Script type="application/ld+json" strategy="beforeInteractive">
        {sm_breadcrumb_list(breadcrumbItems)}
      </Script>
      {includeFaqSchema && (
        <Script type="application/ld+json" strategy="beforeInteractive">
      ...
      <Layout title={metatags[0].title} description={metatags[0].description} segmentName="Home" lang={lang}>
        ...
        <FloatingCtaSectionv2
          ...
          url={'#billingButtons'}
          customText={
            <div className={`flex flex-col items-center gap-4 text-center ${firstCtaTextPadding}`}>
              <p className="text-2xl font-semibold leading-tight text-gray-95 lg:text-4xl">
                {textContent.ctaSection.title}
              </p>
              <p className={firstCtaDescriptionClass}>{textContent.ctaSection.description}</p>
            </div>
          }
        ...
        <Footer textContent={footerLang} lang={locale} breadcrumbItems={breadcrumbItems} />
      </Layout>
    </>
  );
};
