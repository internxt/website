...
interface ItemsNavigationProps {
  ...
  getTitles: Record<string, any>;
  lang: string | undefined;
  darkMode: boolean | undefined;
  isQuizSection: boolean | undefined;
}
...
export const ItemsNavigation = ({
  ...
  getTitles,
  textContent,
}: ItemsNavigationProps): JSX.Element => {
            ...
            className={`whitespace-nowrap py-1.5 px-4 transition duration-150 ease-in-out ${
              darkMode || isQuizSection
                ? `text-white hover:text-cool-gray-20 ${
                    pathname?.split('/')[1] === getTitles.links.pricing.trim().toLowerCase() && 'text-primary'
                  }`
                : pathname?.split('/')[1] === getTitles.links.pricing.trim().toLowerCase()
                ? 'text-primary'
            ...
            className={`whitespace-nowrap py-1.5 px-4 transition duration-150 ease-in-out ${
              darkMode || isQuizSection
                ? `text-white hover:text-gray-20 ${
                    pathname?.split('/')[1] === getTitles.links.about.split(' ')[0].toLowerCase() && 'text-primary'
                  }`
                : pathname?.split('/')[1] === getTitles.links.about.split(' ')[0].toLowerCase()
                ? 'text-primary'
                : 'text-gray-60 hover:text-primary'
            }
                  } text-base font-medium`}
