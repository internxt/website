...
export const PriceCard = ({
  ...
  label,
  isFamilyPage,
  colorUpCard='bg-white',
  colorDownCard=' bg-neutral-10',
  isBlackFriday,
    ...
    <div
      className={`${
        isBlackFriday
          ? ''
          : popular
          ? `border-${colorCard}/50 ring-[3px]`
          : 'ring-1 ring-gray-10'
      } m-2 flex ${cardMaxWidth} flex-shrink-0 flex-grow-0 flex-col overflow-hidden rounded-2xl`}
    >
          <div className={`info flex flex-col items-center justify-center space-y-6 rounded-t-2xl ${colorUpCard} p-6 pt-6`}>
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className={`${labelBackground} flex rounded-full px-3 py-0.5`}>
            <p className={`${isBlackFriday ? 'text-white' : `text-${colorCard}`} text-lg font-medium`}>
              {cardLabel}
            </p>
          </div>
        </div>
        ...
        >
          <div
            ...
            `}
          >
            <p className={` flex flex-row items-start space-x-1 whitespace-nowrap font-medium ${isBlackFriday ? 'text-white' : 'text-gray-100'}`}>
          ...
          <p
            ...
            } flex-row items-start space-x-1 whitespace-nowrap font-semibold ${isBlackFriday ? 'text-white' : 'text-gray-50'} line-through`}
          >
      ...
      <div className={`featureList flex flex-col border-t border-neutral-20 ${colorDownCard} pb-6 text-sm text-gray-80`}>
          ...
          {contentText.productFeatures[productCardPlan][storage].map((feature) => (
            ...
            >
              <Image
                ...
                alt="check icon"
              />
             <span className={`${isBlackFriday ? 'text-white' : 'text-gray-80'}`}>
                {feature}
              </span>
            </div>
          ))}
